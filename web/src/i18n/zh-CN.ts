const zhCN: Record<string, string> = {
  ...
  "profile.denseLoad": "稠密权重加载",
  "profile.expertMatmul": "专家矩阵乘",
  "profile.sharedExpert": "共享专家",
  "profile.router": "路由",
  "profile.attention": "注意力",
  "profile.blockOverhead": "归一化/残差",
  ...
  "profile.throughputTitle": "每轮生成速度",
  ...
  "profile.oneTurn": "1 轮",
  "profile.diskNote": "磁盘服务是读取路由专家的累计耗时；稠密权重加载则单独统计每层非路由权重的同步装载。普通模式可以让专家读取与计算重叠，因此只有计算线程实际感受到的等待才计入总耗时分解；低内存模式会同步读取，磁盘服务时间也会计入 I/O 等待。多 KV 会话时，份额描述的是整个引擎在该轮窗口内的表现。",
